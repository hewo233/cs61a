...
def minimum_mewtations(typed, source, limit):
    ...
    if(typed[-1] == source[-1]):
        ...
        return 1 + min( minimum_mewtations(typed[:-1],source,limit-1) , minimum_mewtations(typed,source[:-1],limit-1), minimum_mewtations(typed[:-1],source[:-1],limit-1))
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times = []
    for each in timestamps_per_player:
        each_times = []
        for x in range(1,len(each)):
            each_times.append(each[x] - each[x - 1])
...
def fastest_words(match):
    ...
    for i in range(len(words)):
        ...
        for j in range(len(times)):
            if(times[j][i] < times[minn][i]):
                minn = j
        ans[minn].append(words[i])
