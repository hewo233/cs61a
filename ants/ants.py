...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while(not now_place.is_hive and now_place):
            if(self.lower_bound <= distance <= self.upper_bound and now_place.bees):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if(self.health <= amount):
            opamount += self.damage
        for bee in place.bees[:]:
            bee.reduce_health(opamount)
        #print("Calling super().reduce_health")
        super().reduce_health(amount)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if ( self.turns_to_chew == 0 ):
            if(place.bees):
                unlucky_bee = random_bee(place.bees)
